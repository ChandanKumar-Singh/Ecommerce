...
class ProductProvider extends ChangeNotifier {
  ...
  Future<void> getAllProducts() async {
    ...
    // print("11111111111111111111111111111111111 ${products.length}");
    // notifyListeners();
    loading = false;
    // notifyListeners();
  }

  Stream<List<ProductModel>> getAllProductsFromDB() async* {
    CollectionReference ref =
        firestore.collection('ecommerce').doc('admin').collection('products');
    ...
    try {
      ...
      await for (var docs in snap) {
        ...
        for (var doc in docs.docs) {
          ...
          var item = ProductModel.fromJson(doc.data() as Map<String, dynamic>);
          // print('mapped');
        ...
        // print('Loading products from db $loading');
        // print(
        //     "0000000000000000000000000000000000000000000000000000000 ${products.length}");
        notifyListeners();
      }
  ...
  Future<void> addProductsToDB(ProductModel product) async {
    await firestore
    ...
    // print('Added ${product.id} to db');
  }

  Future<void> addUsersToDB(Map<String, dynamic> user) async {
    await firestore
        .collection('ecommerce')
        .doc('admin')
        .collection('users')
